...
1. **The launch command.** From a git checkout it's `node /path/to/codex-imagegen-mcp/dist/src/cli.js serve`; after `npm link` or `npm i -g` it's `codex-imagegen-mcp serve`. `codex-imagegen-mcp config <client>` prints the exact snippet for your machine.
